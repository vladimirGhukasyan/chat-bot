<template>
    <div style="display: flex;">
        <div style="display: flex;flex-direction: column;">
            <input type="text" v-model="filter" style="padding: 8px;border: 0;margin-bottom: 6px;border-radius: 4px;">
            <div ref="dropdown" class="dropdown-tool-component">
                <a @click.stop="selectItem($event, s)" v-if="s.match(filter)" v-for="s in data" v-html="s" :class="[selected === s ?'selected' :'']"></a>
            </div>
        </div>
        ...
        methods: {
            selectItem(e, id) {
                if (e.target.getAttribute('data-data') !== undefined && e.target.getAttribute('data-data') !== null)
                    this.$emit('data', this.blockId, e.target.getAttribute('data-data'));
                else this.$emit('data', this.blockId, id);
                this.selected = id;
            }
        },
        data() {
