<template>
    <div ref="imgupload" class="imgupload-tool-component">
        <div @click="uploadImage();" style="cursor: pointer; padding: 10px;display: flex;flex-direction: column;">
            ...
            <div style="color: #2b2b2b; font-size: 10px; text-align: center; margin-top: 10px;">upload video</div>

            <video ref="video" autoplay muted loop style="width: 100%;margin-top: 10px;">
            ...
            this.$refs.fileSelect.addEventListener('change', function () {
                if (this.files && this.files[0]) {
                    ...
                    reader.onload = function (e) {
                        parent.$refs.video.children[0].setAttribute('src', e.target.result);
                        parent.$refs.video.load();
                        parent.$refs.video.play();
                        parent.$emit('data', e.target.result, undefined);
                        // parent.$emit('data', 'background:none');

                        let once = false;
                        parent.$refs.video.addEventListener('playing', function () {
                            if (once) return;
                            setTimeout(function () {
                                parent.$emit('data', undefined, 'videoOnLoaded');
                                once = true;
                            }, 500);
                        });
                    };
