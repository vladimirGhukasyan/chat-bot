<template>
    <div style="display: flex; flex: 1;">
        <div v-if="!createNewAgent" class="agents">
            <div class="agent-header">
                ...
                <div>Password</div>
                <div>
                    <button class="create-new" @click="createNewAgent = true">
                        <div>+</div>
                        NEW
                    </button>
                </div>
            ...
            <div style="border-radius: 0 0 10px 10px;overflow: hidden;">
                <div class="agent" v-for="agent in agentList">
                    <div class="user-photo-1 online online-big">
                        <img :src="agent.photo" :alt="agent.name">
                    </div>
                    <div>{{agent.name}}</div>
                    <div>{{agent.email}}</div>
                    <div>{{agent.login}}</div>
                    <div>{{agent.password}}</div>
                    <div>
                        <a @click="getAgentById(agent.id)" class="edit-button" style="margin-right: 5px;"></a>
                        <a @click="removeAgentById(agent.id)" class="remove-button"></a>
                    </div>
                </div>
            </div>
        </div>
        <div v-if="createNewAgent" class="add-agent">
            <form class="add-agent-container">
                <div class="field-row">
                    <div>
                        <div class="label">Name</div>
                        <input type="text" placeholder="Write name of agent" v-model="tempAgent.name">
                    </div>
                    <div>
                        <div class="label">User Name</div>
                        <input type="text" placeholder="Write user name of agent" v-model="tempAgent.login">
                    </div>
                </div>
                <div class="field-row">
                    <div style="flex: 0;margin-bottom: 0;">
                        <img :src="tempAgent.photo" alt="" style="width: 96px;height: 96px;border-radius: 96px;">
                    </div>
                    <div style="align-self: center;margin-top: 0;">
                        <div class="label">Avatar</div>
                        <div @click="uploadAgentPhoto()" class="input" style="max-height: 18px;">
...
<script>
    export default {
        ...
        methods: {
                ...
                this.$refs.fileSelect.addEventListener('change', function () {
                    if (this.files && this.files[0]) {
                        ...
                        reader.onload = function (e) {
                            parent.tempAgent.photo = e.target.result;
                        };
            ...
            addNewAgent() {
                this.createNewAgent = false;

                if (this.tempAgent.id) {
                    for (let i = 0; i < this.agentList.length; i++) {
                        if (this.agentList[i].id === this.tempAgent.id) {
                            this.agentList[i] = Object.assign(this.tempAgent, {});
                            break;
                        }
                    }
                } else {
                    this.agentList.push({
                        id: 100 + Math.floor(Math.random() * 16777216),
                        photo: this.tempAgent.photo,
                        name: this.tempAgent.name,
                        email: this.tempAgent.email,
                        login: this.tempAgent.login,
                        password: this.tempAgent.password
                    });
                }
                ...
                this.tempAgent = {
                    id: 0,
                    name: '',
                    photo: '/assets/images/image-photo-icon.png',
                    email: '',
                    login: '',
                    password: '',
                    password2: ''
            ...
            getAgentById(id) {
                for (let i = 0; i < this.agentList.length; i++) {
                    if (this.agentList[i].id === id) {
                        this.tempAgent = Object.assign(this.agentList[i], {});
                    }
                }
...
<style lang="scss">
    ...
    .agents {
        flex: 1;
        // box-shadow: 0 5px 4px rgba(255, 255, 255, 0.4);
